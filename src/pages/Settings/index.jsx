  ...
  return (
    <Container>
      <div className='relative flex max-w-[1024px] mx-auto bg-blackMenu dark:bg-bggTop rounded-lg p-3'>
        <div
          className={`${
            activeMenu ? 'translate-x-0' : '-translate-x-[110%]'
          } md:translate-x-0 md:min-w-[260px] absolute top-0 left-0 right-0 bottom-0 rounded-lg   z-[997] md:z-[1] bg-blackMenu dark:bg-bggTop md:static transition-all md:col-span-1 overflow-y-auto border-r border-background dark:border-bggBottom md:h-[calc(100vh-132px)] pr-3`}
        >
          ...
          <div className='flex flex-col'>
            {profileLinks.map((el, i) => (
              <Link
                ...
                to={el.link}
                className={`${
                  activeLink === el.pathname ? styleLink : ''
                } px-3 py-2 rounded-full font-bold mb-2 hover:bg-whiteOpacity dark:hover:bg-bggBottom transition-colors`}
              >
                {el.title}
              </Link>
            ))}
