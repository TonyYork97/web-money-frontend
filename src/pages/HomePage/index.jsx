      ...
      <div className='pb-2'>
        <div className='flex flex-col-reverse gap-3 md:items-center md:flex-row mb-4 justify-between'>
          <div className='flex  flex-col sm:flex-row  gap-4'>
            <Link
              className='w-full md:w-auto rounded-3xl text-center border border-mainGreen py-4 px-4 md:py-6 font-bold  hover:bg-secondBackground  dark:hover:bg-bggTop transition-colors'
              to={links.addExpense}
            >
              Добавить расход
            </Link>
            <Link
              className='w-full md:w-auto rounded-3xl  bg-gradient-to-r from-mainGreen to-bggGreen text-center text-background py-4 md:py-6 px-4 font-bold  hover:from-mainGreen hover:to-mainGreen transition-colors'
              to={links.addRevenue}
            >
              Добавить доход
            </Link>
          ...
          >
            <TodayExpense
              data={todayExpense}
              isLoading={isLoadingTodayExpense}
              reload={reloadTodayExpense}
              error={todayExpenseError}
            />
            <TotalCash
              data={totalCash}
              isLoading={isLoadingTotalCash}
              reload={reloadTotalCash}
              error={totalCashError}
            />
          </div>
        </div>
