...
import { fetchChangeEmail, setChangeEmailError, setIsSuccessChangeEmail } from '../../store/slices/authSlice'
...
export const EmailEdit = () => {
  ...
  const changeEmail = async () => {
    ...
    if (data.user?.email === trimEmail || data.user?.email === trimConfirmEmail) {
      setNewEmailError(true)
    ...
    const response = await dispatch(fetchChangeEmail({ email: trimEmail, confirmEmail: trimConfirmEmail, password }))
    ...
    <div>
      ...
      <div >
        ...
        <div className='flex gap-3 items-center'>
          <ButtonGreen disabled={!isLoadingChangeEmail} func={changeEmail} title="Изменить Email" />
