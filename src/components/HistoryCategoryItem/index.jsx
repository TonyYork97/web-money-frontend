...
export const HistoryCategoryItem = ({
  id,
  title,
  amount,
  date,
  ...
  return (
    <div className='flex gap-2 justify-between bg-background dark:bg-bggTop dark:shadow-blockLight px-2 py-1 rounded-lg border-b boreder-textPrime mb-3'>
      <div className=''>
        ...
        <p className='text-[11px]  text-textOpacity dark:text-textOpacitySecond break-words'>
      ...
      <div className='flex  gap-3 md:gap-4 items-center'>
        <div className='flex flex-col items-end sm:flex-row-reverse sm:gap-3 sm:items-center sm:justify-between'>
          {type === 'revenue' ? (
            <p className='text-mainGreen dark:text-green-700 whitespace-nowrap mb-[1px]'>
              +{amount} &#8381;
            </p>
          ) : (
            ...
            <div className=''>
              {paymentMethodImage ? (
                <img
                  ...
                  src={`https://web-money-backend.onrender.com${paymentMethodImage}`}
                  alt={paymentMethod}
                  title={paymentMethod}
                />
              ) : (
