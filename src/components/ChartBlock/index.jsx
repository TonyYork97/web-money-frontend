...
export const ChartBlock = ({
  ...
  const data = {
    datasets: [
      {
        label: '# of Votes',
        data: monthExpense?.categories
          ? monthExpense.categories.map((el) => el.amount)
          : [],
        backgroundColor: monthExpense?.categories
          ? monthExpense.categories.map((el) => el.colorHex)
          : [],
        borderColor: monthExpense?.categories
          ? monthExpense.categories.map((el) => el.colorHex)
          : [],
        borderWidth: 1,
      },
    ],
    labels: monthExpense?.categories
      ? monthExpense.categories.map((el) => el.title)
      : [],
  }
  const options = {
    plugins: {
          ...
          <div className=''>
            <div className=' mb-5 flex justify-between text-lg font-bold gap-1'>
              <h3 className='capitalize'>
                {title} {monthExpense?.month}
              </h3>
              <p className='whitespace-nowrap'>
                {monthExpense?.totalMonthAmount} &#8381;
              </p>
            </div>
            <div className='max-full mx-auto mb-3 relative'>
              ...
              {monthExpense?.categories?.length ? (
                <div className='max-w-[208px] sm:max-w-[320px] md:max-w-[260px] lg:max-w-[360px] mx-auto'>
                  {' '}
                ...
                <div className='flex flex-col justify-center items-center gap-8 h-[360px] max-w-[360px] mx-auto w-full'>
                  <h3 className='font-bold text-lg max-w-[220px] text-center mx-auto'>
                    Добавь свой первый расход в этом месяце
                  </h3>
                  <Link
                    to={links.addExpense}
                ...
                <div className='flex flex-col justify-center items-center gap-8 h-[360px] max-w-[360px] mx-auto w-full'>
                  <h3 className='font-bold text-lg max-w-[220px] text-center mx-auto'>
                    Добавь свой первый доход в этом месяце
                  </h3>
                  <Link
                    to={links.addRevenue}
            ...
            <ul className='flex flex-col mb-1 flex-1'>
              {monthExpense?.categories
                ? full
                  ? monthExpense.categories?.length
                    ? monthExpense.categories.map((el, i) => (
                        <CategoryItem
                          onClick={handleClick}
                          id={i}
                          key={el.title}
