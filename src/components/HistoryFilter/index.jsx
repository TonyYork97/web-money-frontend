...
export const HistoryFilter = ({
  ...
  const { type, minDateValue, maxDateValue, minAmountValue, maxAmountValue } =
    useSelector((state) => state.filter)
  const dispatch = useDispatch()

  const getFilterOperations = async ({
    minDate,
    maxDate,
    minAmount,
    maxAmount,
  ...
  const handleDate = () => {
    ...
    if (!minDateValue) {
      ...
      setFullDateValue(
        `${moment(minDate).format('DD.MM.YY')} - ${moment(maxDateValue).format(
          'DD.MM.YY'
        )}`
      )
      setIsPeriodActive(false)
    ...
    if (!maxDateValue) {
      ...
      setFullDateValue(
        `${moment(minDateValue).format('DD.MM.YY')} - ${moment(maxDate).format(
          'DD.MM.YY'
        )}`
      )
      setIsPeriodActive(false)
    ...
    getFilterOperations({})
    setFullDateValue(
      `${moment(minDateValue).format('DD.MM.YY')} - ${moment(
        maxDateValue
      ).format('DD.MM.YY')}`
    )
    setIsPeriodActive(false)
  ...
  return (
    <>
      <div
        className={`${
          width > 1132 ? 'overflow-visible' : 'overflow-x-auto'
        } pb-1 mb-3 w-[calc(100%-14px)] absolute ${styles.filterHistory}`}
      >
          ...
          <div className='w-[170px] relative '>
            ...
            {isTypeActive && width > 1132 && (
              <>
                <div
                  onClick={closePeriod}
                  className='fixed z-[990] top-0 right-0 left-0 bottom-0'
                ></div>
              ...
              <>
                ...
                <div className='absolute w-full dark:border dark:border-bggBottom bg-blackMenu dark:bg-white py-3 px-3 z-[997] rounded-lg'>
                  ...
                  <Input
                    ...
                    max={maxDate}
                    value={maxDateValue}
                    onChange={changeMaxDateInput}
                  />
                  <button
                    onClick={handleDate}
                    className='text-mainGreen hover:text-secondGreen dark:text-darkBlack transition-colors'
                  >
                    Продолжить
                  </button>
                </div>
            ...
            {isAmountActive && width > 1132 && (
              <>
                ...
                <div className='absolute w-full z-[990] dark:border dark:border-bggBottom bg-blackMenu dark:bg-white py-3 px-3 rounded-lg'>
                  {amountError && <p className='text-red-700'>{amountError}</p>}
                  <Input
                    label='От'
                    type='number'
      ...
      <div>
        {isAmountActive && width <= 1132 ? (
          <ModalWindow title='Выберете сумму' onClose={closeActive}>
            ...
            <button
              onClick={handleAmount}
              className='text-mainGreen hover:text-bggGreen dark:text-darkBlack dark:hover:text-textPrime transition-colors'
            >
              Продолжить
            </button>
          </ModalWindow>
