...
export const NavMobile = ({ isOpenNav, list = [], closeNav = () => null }) => {
  ...
  return (
    <nav
      className={`${
        isOpenNav ? 'translate-x-0' : 'translate-x-[100%] hidden'
      } ${
        styles.navMobile
      } bg-background  border-black dark:border-bggBottom dark:bg-gradient-to-r dark:from-bggBottom dark:via-bggTop dark:to-white `}
    >
      {list.map((el) => (
        <NavMobileItem
          closeNav={closeNav}
