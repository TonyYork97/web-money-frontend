...
export const AddOperation = () => {
    ...
    const getCategories = async () => {
        if (location.pathname.includes('expense')) {
            await dispatch(fetchGetcategories({ category: 'expense' }));
            await dispatch(fetchGetPaymentMethods());
        } else {
            await dispatch(fetchGetcategories({ category: 'revenue' }));
        }
    }
    ...
    useEffect(() => {
        ...
        setPaymentMethodsArr(dataPaymentMethods ? dataPaymentMethods.map(el => ({ value: el.title, label: <div className='flex gap-3 items-center'><img className='w-12' src={`https://web-money-frontend-pxpz7xzns-tonyyork97.vercel.app${el.imageUrl}`} alt={el.title} /><h3>{el.title}</h3></div> })) : [])
