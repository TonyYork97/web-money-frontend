...
export const History = ({
  ...
  const funcData = useMemo(() => {
    ...
    const dataWithDate = data.length
      ? data.map((el, idx) => {
  ...
  const funcDataWithoutDate = useMemo(() => {
    ...
    const dataWhithoutDate = data.length
      ? data.map((el, idx) => {
          return (
            <HistoryCategoryItem
              key={el._id}
  ...
  return (
    <>
      {deleteSuccess && (
        <PopupWindow
          onClose={() => dispatch(setDeleteSuccess(false))}
          ...
          <div>
            ...
            {full ? (
              <div className='h-1 w-full flex justify-center' ref={observerRef}>
                {isLazyLoading ? <MainLoading size={32} /> : ''}
              </div>
            ) : (
              ''
            )}
        ...
        {data.length ? (
          !full ? (
            <div className='text-right py-1'>
              <ShowMoreButton to={links.history} title='Показать больше' />
            </div>
          ) : (
            ''
          )
